...
export default function Navbar() {
  ...
  const cartQuantity = Math.max(0, cart.reduce((total, item) => total + item.quantity, 0));
  const wishlistCount = wishlist.length;

  const categories = ['Herbal Soaps', 'Essential Oils', 'Gift Boxes', 'Souvenirs'];

  // Match our original page key system with React Router path names
  const getPageKey = () => {
  ...
  const isAtHomeTop = currentPage === 'home' && !isScrolled;

  // Header dynamic placement and premium backing
  const headerClass = currentPage === 'home'
    ? `fixed top-0 left-0 right-0 z-40 transition-all duration-500 w-full ${
        isScrolled 
          ? 'bg-brand-cream/95 backdrop-blur-md border-b border-brand-beige/30 shadow-md' 
  ...
  const linkTextClass = (active: boolean) => {
    if (active) return 'text-brand-gold font-medium';
    return isAtHomeTop 
  ...
  return (
    <header className={headerClass} id="app-nav-header">
      ...
      >
        <span className="truncate">✨ Complimentary Sandalwood &amp; Satin Ribbon Gifting wraps with all order values &bull; Free courier shipping over ₹6,000 ✨</span>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* --- NAVBAR SECTION 1 (UPPER ROW) --- */}
        <div className={`grid grid-cols-3 items-center min-h-[64px] md:min-h-[110px] py-1.5 md:py-2 border-b transition-colors duration-500 ${
          ...
          <div className="flex items-center justify-start space-x-2 sm:space-x-3 md:space-x-6">
            ...
            <div className={`hidden sm:flex items-center space-x-4 text-[10.5px] uppercase tracking-widest font-sans-poppins font-light transition-colors duration-500 ${storeTextClass}`}>
              <span className="flex items-center space-x-1.5 select-none cursor-pointer">
                ...
                <span className="hover:text-brand-gold transition-colors">Our Stores</span>
              </span>
              <span className={`transition-colors ${isAtHomeTop ? 'text-white/20' : 'text-brand-beige/40'}`}>|</span>
          ...
          <div className="flex items-center justify-center text-center">
            <Link 
              to="/" 
              className="flex flex-row items-center justify-center space-x-2 sm:space-x-2.5 md:space-x-3.5 group focus:outline-none"
              id="brand-logo-btn"
            >
              <div className="transition-all duration-300 transform group-hover:scale-105 shrink-0">
                <LavishLathersLogo className="h-5.5 w-5.5 sm:h-6.5 sm:w-6.5 md:h-8 md:w-8" showBackground={true} />
              </div>
              <div className="flex flex-col items-start justify-center text-left mt-0">
                <span className={`block font-serif-playfair text-[10px] xs:text-xs sm:text-[14px] md:text-base lg:text-lg tracking-[0.12em] md:tracking-[0.16em] uppercase font-light transition-colors duration-500 leading-none ${
                  isAtHomeTop ? 'text-brand-cream group-hover:text-brand-gold' : 'text-brand-black group-hover:text-brand-gold'
                }`}>
                  Lavish Lathers
                </span>
          ...
          <div className="flex items-center justify-end space-x-1.5 sm:space-x-3.5 md:space-x-5">
            <nav className="hidden lg:flex space-x-5 items-center mr-2">
              <Link
                to="/about"
                className={`text-[11px] uppercase tracking-widest font-sans-poppins transition-colors relative py-1 ${linkTextClass(currentPage === 'about')}`}
                id="nav-about-upper"
              >
                Our Story
                {currentPage === 'about' && (
              ...
              <Link
                to="/contact"
                className={`text-[11px] uppercase tracking-widest font-sans-poppins transition-colors relative py-1 ${linkTextClass(currentPage === 'contact')}`}
            ...
            <button
              onClick={() => {
                alert("Lavish Lathers Concierge Assistant is ready to customize your orders! Wishlisted items will remain highlighted for easy bulk quoting.");
              }}
              className={`relative p-1.5 transition-colors duration-300 ${iconColorClass} hover:text-brand-gold group`}
              id="wishlist-btn"
            >
            ...
            <button
              ...
              className={`relative py-1 px-2.5 sm:py-1.5 sm:px-3 rounded-full transition-all duration-500 flex items-center space-x-1 sm:space-x-1.5 text-[9px] sm:text-[10px] font-sans-poppins tracking-wider ${
                isAtHomeTop 
            ...
            >
              ...
              <span className="hidden sm:inline uppercase font-semibold">Cart</span>
              <span className={`text-[8px] sm:text-[9px] font-bold px-1 py-0.1 sm:px-1.5 rounded-full ${
                isAtHomeTop ? 'bg-black/10 text-brand-black' : 'bg-white/20 text-brand-cream'
              }`}>
                {cartQuantity}
              </span>
            </button>
        ...
        {/* Main navigation categories - Centered on desktop, horizontally scrollable on mobile */}
          ...
          <div className="flex items-center space-x-6 sm:space-x-8 md:space-x-10 lg:space-x-12 py-3 md:py-4 overflow-x-auto scrollbar-none w-full justify-start md:justify-center -mx-4 md:mx-0 px-4 md:px-0">
            {categories.map((category) => (
              <button
                ...
                className={`text-[9.5px] sm:text-[10.5px] md:text-xs uppercase tracking-[0.25em] font-sans-poppins transition-colors duration-300 hover:text-brand-gold relative py-1 shrink-0 ${
                  currentPage === 'shop' && filterCategory === category
      ...
      {/* Mobile Drawer */}
      {isOpen && (
        <div className={`md:hidden absolute w-full left-0 z-30 shadow-2xl border-t transition-all duration-500 animate-slide-down ${
          isAtHomeTop 
          ...
          <div className="p-4 space-y-3">
            ...
            <Link
              ...
              className={`block w-full text-left py-1 text-xs uppercase tracking-widest font-sans-poppins hover:text-brand-gold font-light transition-colors ${
            ...
            <div className="grid grid-cols-2 gap-2">
              {categories.map((category) => (
                <button
                  ...
                  className={`text-left p-3.5 border rounded-lg text-xs uppercase tracking-widest font-sans-poppins hover:text-brand-gold hover:border-brand-gold/60 transition-all ${
                    currentPage === 'shop' && filterCategory === category 
          ...
          <div className={`p-4 mx-4 border-t my-2 flex justify-between items-center text-[10px] font-sans-poppins ${
            isAtHomeTop ? 'border-white/10 text-white/50' : 'border-brand-beige/25 text-brand-black/60'
          }`}>
