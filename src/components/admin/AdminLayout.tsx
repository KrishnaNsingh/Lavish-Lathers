...
import { LayoutDashboard, ShoppingBag, ClipboardList, LogOut, ArrowLeft, Heart } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0C0B0A] text-brand-cream font-sans-inter flex flex-col md:flex-row text-left">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-1">
            <div className="flex items-center space-x-2 text-brand-gold">
              ...
              <span className="text-[10px] uppercase font-bold tracking-[0.3em]">Vault Console</span>
            </div>
            <h2 className="font-serif-playfair text-xl text-brand-cream tracking-wide font-semibold">Lavish Atelier</h2>
          </div>
      ...
      <main className="flex-1 p-6 sm:p-10 lg:p-12 overflow-y-auto">
        ...
        <div className="border-b border-brand-cream/10 pb-6 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-[10px] uppercase tracking-[0.4em] text-brand-gold font-sans-poppins font-bold block mb-1">administrative curation</span>
            <h1 className="font-serif-playfair text-3xl text-brand-cream tracking-wide font-normal font-semibold leading-tight">{title}</h1>
            {subtitle && <p className="text-xs text-brand-cream/55 font-light mt-1 max-w-xl">{subtitle}</p>}
          </div>
        ...
        {/* Child templates */}
        <div className="animate-fade-in relative z-15">
          {children}
        </div>
