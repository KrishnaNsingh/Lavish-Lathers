  ...
  const filteredProducts = useMemo(() => {
    let result = products.filter((p) => {
      ...
      const matchesCategory =
        filterCategory === "All" ||
        (filterCategory === "Souvenirs" &&
          (p.artistryType === "Keepsake Souvenir" || p.souvenir)) ||
        (filterCategory === "Skincare" &&
          p.artistryType === "Skincare formulation") ||
      ...
      const matchesCollectible =
        ...
        (activeCollectibleFilter && p.souvenir) ||
        (!activeCollectibleFilter && !p.souvenir);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-8 bg-brand-cream border border-brand-beige/25 p-6 sm:p-8 rounded-[2rem] text-left shadow-lg shadow-brand-black/[0.01]">
            ...
            <div className="space-y-3 font-sans-poppins">
              <label className="block text-[10px] uppercase tracking-widest text-brand-black/50 font-bold">
                Curation Categories
              </label>
              <div className="flex flex-col space-y-1.5 pl-1">
                {categories.map((cat) => (
                  ...
                  >
                    <span>{cat}</span>
                    <span
                      className={`text-[10px] transform transition-transform ${filterCategory === cat ? "rotate-90 text-brand-gold" : "text-brand-black/30"}`}
                    >
                      ▶
                    </span>
                  </button>
